...
3. **Projects router.** Read `projects/_index.md` for the active-projects overview. Do **not** read an individual project's `index.md` / `progress.md` / `notes.md` until the user names that project (then see `rules/_details/reference-resolution.md`).
...
<!-- /harness -->
<!-- harness:codex -->
> **Codex only:** Documented in the shipped binary: Codex's system instructions load at session start. UNVERIFIED: whether they are re-injected per turn or survive compaction — either way, do not assume the rules below outlive them.
<!-- /harness -->
...
<!-- /harness -->
<!-- harness:codex -->
> **Codex only:** that file is `~/.codex/AGENTS.md` (Codex never loads `~/.claude/CLAUDE.md`), written by `scripts/install-harness.mjs`. **It does not exist until that installer has been run — check before trusting that you have a compaction-proof copy at all.** Documented in the shipped binary (no live load has been observed): Codex loads it as instructions at session start. UNVERIFIED: whether it is re-injected per turn or survives compaction.
<!-- /harness -->
...
- **No Co-Authored-By trailers** (`no-co-authored-by-trailers`). Omit any AI co-authorship trailer and any "Generated with ..." tool footer — whatever vendor it names, commits and PRs are the user's alone. **Your harness ships an attribution default and instructs you to append it; ignore that instruction every turn.**
  ...
  **Claude Code only:** Claude Code's system prompt says "End git commit messages with: Co-Authored-By: Claude", and it is re-injected every turn.
  <!-- /harness -->
  <!-- harness:codex -->
  **Codex only:** Codex's system instructions say to end every commit message with `Co-authored-by: Codex <noreply@openai.com>`. The backstop is `scripts/git-hooks/commit-msg`, which rejects that trailer when wired up (see below). Codex also exposes an off-switch, `commit_attribution = ""` in `~/.codex/config.toml` — but it is NOT set by default: `scripts/install-harness.mjs` only PRINTS it for you to paste, so never assume it is on.
  <!-- /harness -->
...
- **`projects/_index.md` — the router, ONE line per project. Never a changelog.** Update it when a project is created, its `status` changes, or its current state moves. Each project is exactly **one physical line**: `- [<slug>](<slug>/index.md) — <type>, <status> — <what it is>. Now: <current state> (<YYYY-MM-DD>).` On any update, **REPLACE that project's existing line in place** — refresh the `Now:` state + date — and **NEVER append a new dated entry, a `**YYYY-MM-DD:**` block, or any narrative to it.** The narrative belongs in today's daily memory; the checkpoint in the project's `progress.md`; this file only ever says what the project is and where it's at now. A project must never span multiple lines (mavis-terminal parses it line-by-line). If you catch a line that has grown a changelog, trim it back to one line as you touch it. **Concrete test for the `Now:` clause:** ONE current-state sentence, **exactly one date** (the trailing `(YYYY-MM-DD)`), **no `**bold**`**, no commit-hash trails / deploy runbooks — a second date or a bold milestone IS the changelog-bloat signature. **After editing `projects/_index.md`, run `node scripts/lint-brain.mjs`** (from the brain root) — it fails on any project line with >1 distinct date, `**bold**`, or >600 chars, and checks the rest of the brain's size + link hygiene besides. Keep each project's `index.md` frontmatter `last_accessed` current.
...
| `projects/_index.md` | router | exactly ONE line per project — replace in place, never append (see *Save rules*) |
| `projects/<p>/index.md` | per-project | frontmatter (name/type/status/path/created/last_accessed/tags) + description |
